...
def ball_animation():
    global ball_speed_x, ball_speed_y, collision_counter, respawn_counter, diamond_counter, level
    ...
    for diamond in diamonds[:]:  # Durch eine Kopie der Liste iterieren, um während des Iterierens Elemente entfernen zu können
        if ball.colliderect(diamond):
            ...
            # Wenn alle Diamanten getroffen wurden, erhöhe das Level und spawn neue Diamanten
            if diamond_counter == 10:
                level += 1
                diamond_counter = 0
                spawn_diamonds()
