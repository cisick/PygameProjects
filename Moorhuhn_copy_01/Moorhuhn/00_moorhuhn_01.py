...
pygame.display.set_caption('Moorhuhn Spiel')
...
chicken_image = pygame.image.load('Moorhuhnbilder/Moorhuhn_01.png')
chicken_image = pygame.transform.scale(chicken_image, (40, 40))  # Huhn auf 40x40 skalieren
...
def show_menu():
    window.fill(BLACK)
    menu_text = MENU_FONT.render('Moorhuhn Spiel', True, WHITE)
    start_text = MENU_FONT.render('Neues Spiel starten', True, WHITE)
    menu_text_rect = menu_text.get_rect(center=(WINDOW_WIDTH // 2, WINDOW_HEIGHT // 2 - 50))
        ...
        if event.type == pygame.QUIT:
            ...
            if game_running:
                ...
                if event.key == pygame.K_r and ammo < 8:
                    ammo = 8
                # Öffne das Menü mit der Escape-Taste
