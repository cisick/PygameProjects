...
def init():
    global all_sprites, chickens, score, ammo, playing, option_menu
...
# Funktion zum Speichern der Punktzahl
def save_score(player):
    ...
    with open('highscores.dat', 'wb') as file:
        pickle.dump(highscores[:10], file)

# Funktion zum Anzeigen der besten 10 Punktzahlen
def display_highscores(screen, font):
    try:
        with open('highscores.dat', 'rb') as file:
            highscores = pickle.load(file)
            screen.fill(GREEN)
            screen.blit(font.render("Highscores:", True, BLACK), (50, 50))
            for i, player in enumerate(highscores, 1):
                screen.blit(font.render(f"{i}. {player.name}: {player.score}", True, BLACK), (50, 50 + i * 30))
    except FileNotFoundError:
        screen.fill(WHITE)
        screen.blit(font.render("Noch keine Highscores vorhanden.", True, BLACK), (50, 50))
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if start_text_rect.collidepoint(event.pos):
                ...
                print("Highscore")
                display_highscores(window, font)
                # Erweiterung hier!
    ...
    if game_running:
        ...
        if playing:
            if current_time >= game_end_timer:
                ...
                game_running = False
                menu_shown = True
                # Spielerpunktzahl erfassen (angenommen, das Spiel ist beendet und der Spieler hat eine Punktzahl erzielt)
                player_score = score # Beispielwert, ersetze ihn durch deine tatsächliche Punktzahl
                # Spieler erstellen
                player_name = input_username()
                player = Player()
                player.name = player_name
                player.score = player_score
                # Punktzahl speichern
                save_score(player)
                # Beste 10 Punktzahlen anzeigen
                display_highscores(window, font)
