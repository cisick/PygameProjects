...
ballon_image = pygame.image.load('Moorhuhnbilder/Ballon_01.png')
ballon_image = pygame.transform.scale(ballon_image, (90, 140))  # Huhn auf 100x50 skalieren
...
def init():
    global all_sprites, chickens, score, ammo, playing, option_menu, highscore_menu, menu_shown, chicken_kill, ballon_kill
    pygame.display.set_caption("Moorhuhn Jagd")
    playing = True
    option_menu = False
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if ammo > 0:
                ...
                clicked_sprites = [sprite for sprite in ballons if sprite.rect.collidepoint(event.pos)]
                if clicked_sprites:
    ...
    if game_running:
        ...
        if playing:
            ...
            if current_time >= game_end_timer:
                ...
                if len(ballons) < NUM_BALLONS:
                    if len(ballons) % 2 == 0:
                        ballon = Ballon("left")  # Huhn fliegt von links nach rechts
                    else:
                        ballon = Ballon("right")  # Huhn fliegt von rechts nach links
                    all_sprites.add(ballon)
                    ballons.add(ballon)
